  ...
  def index
   ...
   if @statement_data && @statement_data["transacoes"]
    ...
    transactions = transactions.sort_by do |t|
      begin
        Date.parse(t["dataEntrada"])
      rescue
        Date.new(1900, 1, 1)
      end
    end.reverse

    # Atualiza no hash já ordenado
    @statement_data["transacoes"] = transactions

    # Totais
    @total_inflow = transactions.select { |t| t["tipoOperacao"] == "C" }
                                .sum { |t| t["valor"].to_f }

    @total_outflow = transactions.select { |t| t["tipoOperacao"] == "D" }
                                  .sum { |t| t["valor"].to_f }
